...
SELECT name as 'Category Type', title 
FROM film
...
SELECT SUM(p.amount) AS 'Revenue', i.store_id 
FROM payment p
...
SELECT 
    category.name, SUM(payment.amount) AS 'Gross'
FROM
    category
        JOIN
...
CREATE VIEW top_five_genres AS 
		SELECT 
		category.name, SUM(payment.amount) AS 'Total Revenue'
	FROM
		category
			JOIN
		film_category ON category.category_id = film_category.category_id
